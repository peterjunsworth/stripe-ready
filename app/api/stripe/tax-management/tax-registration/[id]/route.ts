...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const response = await updateTaxRegistration(id, taxData);
        const taxRegistration = response instanceof Response ? await response.json() : response;
        return NextResponse.json({ success: true, taxRate: taxRegistration?.taxRegistration });
    } catch (error: any) {
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        return NextResponse.json({ success: true, taxRate: deletedTaxRegistration?.taxRegistration });
    } catch (error: any) {
